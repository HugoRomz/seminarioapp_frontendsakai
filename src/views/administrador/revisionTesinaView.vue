...
const getAllTesinas = async (periodoId) => {
    ...
    try {
        ...
        if (tesinas.value.length === 0) {
            toast.open({
                message: 'No hay tesinas registradas en este periodo',
                type: 'info'
            });
        }
    } catch (error) {
...
const loadUsers = async () => {
    loading.value = true;
    try {
        const response = await TesinaApi.getDocentes();
...
const aceptarTesina = async () => {
    ...
    try {
        ...
        await getDocentesConTesinas(selectedPeriodos.value.value.periodo_id);
    } catch (error) {
        toast.open({
        ...
        <div class="col-12">
            <card class="mb-4">
                <template #content>
                    ...
                    <br>
                    <div v-if="docentesTesinas && Object.keys(docentesTesinas).length !== 0">
                        <div class="col-12">
                            <Fieldset class="mb-3" legend="Docentes Asignados" :toggleable="true" :collapsed="true">
                                <div class="card bg-white shadow-xl rounded-lg p-5 border border-gray-200">
                                    <DataTable
                                        ...
                                        v-model:filters="docentesFilters"
                    ...
                    <div v-else>
                        <card>
                            <template #title>Docentes Asignados</template>
                            <template #content> Selecciona un periodo para ver los docentes asignados. </template>
                        </card>
                    </div>
                    <br>
                    <div v-if="tesinas && Object.keys(tesinas).length !== 0">
                        <div class="col-12">
                    ...
                    <div v-else>
                        <card>
                            <template #title>Tesinas Registradas</template>
                            <template #content> Selecciona un periodo para ver las tesinas registradas. </template>
                        </card>
                    </div>
                </template>
            </card>
